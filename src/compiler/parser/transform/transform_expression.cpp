...
std::unique_ptr<ParsedExpression> Transformer::transformComparisonExpression(
  ...
  if (1 == ctx.nEUG_BitwiseOrOperatorExpression().size()) {
    return transformBitwiseOrOperatorExpression(
        *ctx.nEUG_BitwiseOrOperatorExpression(0));
  }
  ...
  NEUG_ASSERT(ctx.nEUG_ComparisonOperator().size() == 1);
  auto left = transformBitwiseOrOperatorExpression(
      *ctx.nEUG_BitwiseOrOperatorExpression(0));
  auto right = transformBitwiseOrOperatorExpression(
      *ctx.nEUG_BitwiseOrOperatorExpression(1));
  auto comparisonOperator = ctx.nEUG_ComparisonOperator()[0]->getText();
  if (comparisonOperator == "=") {
    return std::make_unique<ParsedExpression>(ExpressionType::EQUALS,
                                              std::move(left), std::move(right),
                                              ctx.getText());
  } else if (comparisonOperator == "<>") {
    return std::make_unique<ParsedExpression>(ExpressionType::NOT_EQUALS,
                                              std::move(left), std::move(right),
                                              ctx.getText());
  } else if (comparisonOperator == ">") {
    return std::make_unique<ParsedExpression>(ExpressionType::GREATER_THAN,
                                              std::move(left), std::move(right),
                                              ctx.getText());
  } else if (comparisonOperator == ">=") {
    return std::make_unique<ParsedExpression>(
        ExpressionType::GREATER_THAN_EQUALS, std::move(left), std::move(right),
        ctx.getText());
  } else if (comparisonOperator == "<") {
    return std::make_unique<ParsedExpression>(ExpressionType::LESS_THAN,
                                              std::move(left), std::move(right),
                                              ctx.getText());
  } else {
    NEUG_ASSERT(comparisonOperator == "<=");
...
Transformer::transformBitwiseOrOperatorExpression(
    CypherParser::NEUG_BitwiseOrOperatorExpressionContext& ctx) {
  std::unique_ptr<ParsedExpression> expression;
  for (auto i = 0ul; i < ctx.nEUG_BitwiseAndOperatorExpression().size(); ++i) {
    auto next = transformBitwiseAndOperatorExpression(
        *ctx.nEUG_BitwiseAndOperatorExpression(i));
    if (!expression) {
      expression = std::move(next);
    } else {
      auto rawName = expression->getRawName() + " | " + next->getRawName();
      expression = std::make_unique<ParsedFunctionExpression>(
          BitwiseOrFunction::name, std::move(expression), std::move(next),
          rawName);
    }
  }
  return expression;
...
std::unique_ptr<ParsedExpression>
Transformer::transformBitwiseAndOperatorExpression(
    CypherParser::NEUG_BitwiseAndOperatorExpressionContext& ctx) {
  std::unique_ptr<ParsedExpression> expression;
  for (auto i = 0ul; i < ctx.nEUG_BitShiftOperatorExpression().size(); ++i) {
    auto next = transformBitShiftOperatorExpression(
        *ctx.nEUG_BitShiftOperatorExpression(i));
    if (!expression) {
      expression = std::move(next);
    } else {
      auto rawName = expression->getRawName() + " & " + next->getRawName();
      expression = std::make_unique<ParsedFunctionExpression>(
          BitwiseAndFunction::name, std::move(expression), std::move(next),
          rawName);
    }
  }
  return expression;
...
std::unique_ptr<ParsedExpression>
Transformer::transformBitShiftOperatorExpression(
    CypherParser::NEUG_BitShiftOperatorExpressionContext& ctx) {
  std::unique_ptr<ParsedExpression> expression;
  for (auto i = 0ul; i < ctx.oC_AddOrSubtractExpression().size(); ++i) {
    auto next =
        transformAddOrSubtractExpression(*ctx.oC_AddOrSubtractExpression(i));
    if (!expression) {
      expression = std::move(next);
    } else {
      auto bitShiftOperator = ctx.nEUG_BitShiftOperator(i - 1)->getText();
      auto rawName = expression->getRawName() + " " + bitShiftOperator + " " +
                     next->getRawName();
      if (bitShiftOperator == "<<") {
        expression = std::make_unique<ParsedFunctionExpression>(
            BitShiftLeftFunction::name, std::move(expression), std::move(next),
            rawName);
      } else {
        NEUG_ASSERT(bitShiftOperator == ">>");
        expression = std::make_unique<ParsedFunctionExpression>(
            BitShiftRightFunction::name, std::move(expression), std::move(next),
            rawName);
      }
    }
  }
...
std::unique_ptr<ParsedExpression> Transformer::transformAddOrSubtractExpression(
  ...
  for (auto i = 0ul; i < ctx.oC_MultiplyDivideModuloExpression().size(); ++i) {
    auto next = transformMultiplyDivideModuloExpression(
        *ctx.oC_MultiplyDivideModuloExpression(i));
    if (!expression) {
      expression = std::move(next);
    } else {
      auto arithmeticOperator =
          ctx.nEUG_AddOrSubtractOperator(i - 1)->getText();
      auto rawName = expression->getRawName() + " " + arithmeticOperator + " " +
                     next->getRawName();
      expression = std::make_unique<ParsedFunctionExpression>(
...
std::unique_ptr<ParsedExpression>
Transformer::transformMultiplyDivideModuloExpression(
  ...
  for (auto i = 0ul; i < ctx.oC_PowerOfExpression().size(); i++) {
    ...
    if (!expression) {
      ...
      auto arithmeticOperator =
          ctx.nEUG_MultiplyDivideModuloOperator(i - 1)->getText();
      auto rawName = expression->getRawName() + " " + arithmeticOperator + " " +
                     next->getRawName();
      expression = std::make_unique<ParsedFunctionExpression>(
          arithmeticOperator, std::move(expression), std::move(next), rawName);
...
std::unique_ptr<ParsedExpression> Transformer::transformPowerOfExpression(
  ...
  std::unique_ptr<ParsedExpression> expression;
  for (auto& unaryAddOrSubtractExpression :
       ctx.oC_UnaryAddSubtractOrFactorialExpression()) {
    auto next = transformUnaryAddSubtractOrFactorialExpression(
...
std::unique_ptr<ParsedExpression>
Transformer::transformUnaryAddSubtractOrFactorialExpression(
    CypherParser::OC_UnaryAddSubtractOrFactorialExpressionContext& ctx) {
  auto result = transformStringListNullOperatorExpression(
...
std::unique_ptr<ParsedExpression>
Transformer::transformStringListNullOperatorExpression(
...
std::unique_ptr<ParsedExpression>
Transformer::transformStringOperatorExpression(
  ...
  if (ctx.STARTS()) {
    return std::make_unique<ParsedFunctionExpression>(
        StartsWithFunction::name, std::move(propertyExpression),
        std::move(right), rawExpression);
  } else if (ctx.ENDS()) {
    return std::make_unique<ParsedFunctionExpression>(
        EndsWithFunction::name, std::move(propertyExpression), std::move(right),
        rawExpression);
...
std::unique_ptr<ParsedExpression> Transformer::transformListOperatorExpression(
  ...
  if (ctx.IN()) {  // x IN y
    auto listContains = std::make_unique<ParsedFunctionExpression>(
        ListContainsFunction::name, std::move(raw));
  ...
  auto listExtract = std::make_unique<ParsedFunctionExpression>(
      ListExtractFunction::name, std::move(raw));
  listExtract->addChild(std::move(child));
  NEUG_ASSERT(ctx.oC_Expression().size() == 1);
...
std::unique_ptr<ParsedExpression> Transformer::transformNullOperatorExpression(
    ...
    std::unique_ptr<ParsedExpression> propertyExpression) {
  auto rawExpression = propertyExpression->getRawName() + " " + ctx.getText();
  NEUG_ASSERT(ctx.IS() && ctx.NULL_());
  return ctx.NOT()
             ? std::make_unique<ParsedExpression>(ExpressionType::IS_NOT_NULL,
                                                  std::move(propertyExpression),
                                                  rawExpression)
...
std::unique_ptr<ParsedExpression>
Transformer::transformPropertyOrLabelsExpression(
...
std::unique_ptr<ParsedExpression> Transformer::transformAtom(
...
std::unique_ptr<ParsedExpression> Transformer::transformListLiteral(
  ...
  auto listCreation = std::make_unique<ParsedFunctionExpression>(
      ListCreationFunction::name, ctx.getText());
...
std::unique_ptr<ParsedExpression> Transformer::transformParameterExpression(
  ...
  auto parameterName = ctx.oC_SymbolicName() ? ctx.oC_SymbolicName()->getText()
                                             : ctx.DecimalInteger()->getText();
  return std::make_unique<ParsedParameterExpression>(parameterName,
                                                     ctx.getText());
}
...
std::unique_ptr<ParsedExpression> Transformer::transformFunctionInvocation(
  ...
  if (ctx.STAR()) {
    return std::make_unique<ParsedFunctionExpression>(CountStarFunction::name,
                                                      ctx.getText());
  }
  ...
  if (ctx.CAST()) {
    for (auto& functionParameter : ctx.nEUG_FunctionParameter()) {
      expression->addChild(
          transformFunctionParameterExpression(*functionParameter));
    }
...
std::string Transformer::transformFunctionName(
...
std::unique_ptr<ParsedExpression> Transformer::transformLambdaParameter(
  ...
  auto vars = transformLambdaVariables(*ctx.nEUG_LambdaVars());
  auto lambdaOperation = transformExpression(*ctx.oC_Expression());
  return std::make_unique<ParsedLambdaExpression>(
      std::move(vars), std::move(lambdaOperation), ctx.getText());
}

std::unique_ptr<ParsedExpression>
Transformer::transformFunctionParameterExpression(
    CypherParser::NEUG_FunctionParameterContext& ctx) {
  if (ctx.nEUG_LambdaParameter()) {
    return transformLambdaParameter(*ctx.nEUG_LambdaParameter());
  } else {
...
std::unique_ptr<ParsedExpression> Transformer::transformPathPattern(
  ...
  auto subquery = std::make_unique<ParsedSubqueryExpression>(
      SubqueryType::EXISTS, ctx.getText());
  auto patternElement =
      PatternElement(transformNodePattern(*ctx.oC_NodePattern()));
...
std::unique_ptr<ParsedExpression> Transformer::transformExistCountSubquery(
  ...
  auto subquery =
      std::make_unique<ParsedSubqueryExpression>(type, ctx.getText());
  subquery->setPatternElements(transformPattern(*ctx.oC_Pattern()));
  if (ctx.oC_Where()) {
    subquery->setWhereClause(transformWhere(*ctx.oC_Where()));
  }
  if (ctx.nEUG_Hint()) {
    subquery->setHint(transformJoinHint(*ctx.nEUG_Hint()->nEUG_JoinNode()));
  }
  return subquery;
}
...
std::unique_ptr<ParsedExpression> Transformer::transformOcQuantifier(
  ...
  auto variable = transformVariable(
      *ctx.oC_FilterExpression()->oC_IdInColl()->oC_Variable());
  auto whereExpr = transformWhere(*ctx.oC_FilterExpression()->oC_Where());
  auto lambdaRaw = variable + "->" + whereExpr->toString();
  auto lambdaExpr = std::make_unique<ParsedLambdaExpression>(
      std::vector<std::string>{variable}, std::move(whereExpr), lambdaRaw);
  ...
  auto listExpr = transformExpression(
      *ctx.oC_FilterExpression()->oC_IdInColl()->oC_Expression());
  return std::make_unique<ParsedFunctionExpression>(
      quantifierName, std::move(listExpr), std::move(lambdaExpr),
      ctx.getText());
}
...
std::unique_ptr<ParsedExpression> Transformer::createPropertyExpression(
    ...
    std::unique_ptr<ParsedExpression> child) {
  auto key = ctx.STAR() ? InternalKeyword::STAR
...
std::unique_ptr<ParsedExpression> Transformer::transformCaseExpression(
  ...
  if (ctx.ELSE()) {
    if (ctx.oC_Expression().size() == 1) {
      ...
      NEUG_ASSERT(ctx.oC_Expression().size() == 2);
  ...
  auto parsedCaseExpression =
      std::make_unique<ParsedCaseExpression>(ctx.getText());
  ...
  for (auto& caseAlternative : ctx.oC_CaseAlternative()) {
    parsedCaseExpression->addCaseAlternative(
        transformCaseAlternative(*caseAlternative));
  }
...
std::unique_ptr<ParsedExpression> Transformer::transformNumberLiteral(
  ...
  if (ctx.oC_IntegerLiteral()) {
    ...
    NEUG_ASSERT(ctx.oC_DoubleLiteral());
...
std::string Transformer::transformPropertyKeyName(
