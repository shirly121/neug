...
class NEUG_API RuleCatalogEntry final : public CatalogEntry {
 public:
  RuleCatalogEntry(std::string name, std::unique_ptr<optimizer::LogicalRule> rule)
      : CatalogEntry{CatalogEntryType::RULE_ENTRY, std::move(name)},
        plan_rule_{std::move(rule)} {}

  optimizer::LogicalRule* getRule() const { return plan_rule_.get(); }

 private:
  std::unique_ptr<optimizer::LogicalRule> plan_rule_;
};
