...
class NEUG_API LogicalOperator {
 public:
  ...
  explicit LogicalOperator(LogicalOperatorType operatorType,
                           std::shared_ptr<LogicalOperator> left,
  ...
  std::shared_ptr<LogicalOperator> getChild(uint64_t idx) const {
    return children[idx];
  }
  ...
  void setCardinality(common::cardinality_t cardinality_) {
 ...
 protected:
  ...
  void copyChildSchema(uint32_t idx) {
    schema = children[idx]->getSchema()->copy();
  }
